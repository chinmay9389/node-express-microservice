...
const proxyOptions = {
  proxyReqPathResolver: (req) => {
    logger.info("[proxyReqPathResolver] originalUrl:", req.originalUrl);
    const resolvedPath = req.originalUrl.replace(/^\/v1/, "/api");
    logger.info("[proxyReqPathResolver] resolvedPath:", resolvedPath);
    return resolvedPath;
  },
  proxyErrorHandler: (err, res, next) => {
    logger.error("[proxyErrorHandler] Proxy error:", err.message);
    logger.error("[proxyErrorHandler] Stack:", err.stack);
    if (err && err.code) {
      logger.error("[proxyErrorHandler] Error code:", err.code);
    }
    res.status(500).json({
      success: false,
